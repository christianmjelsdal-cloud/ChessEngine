...
Binary format (matching NNUETrainer.cpp saveTrainingData):
  ...
  For each position:
    ...
    float   searchEval  (centipawns, white's POV, approximate material count)
...
def material_eval(board: chess.Board) -> float:
    """Simple material count in centipawns, white's POV."""
    score = 0
    for sq in chess.SQUARES:
        p = board.piece_at(sq)
        if p:
...
def append_binary(positions: list, filename: str):
    ...
    with open(filename, "r+b") as f:
        ...
        f.seek(0)
        f.write(struct.pack("<I", existing + len(positions)))
    ...
    print(f"  Appended {len(positions):,} positions (total {existing + len(positions):,}) → {filename}")
...
def process_pgn_file(filepath: str, **kwargs) -> list:
    """Read a .pgn file and process it."""
    with open(filepath, "r", encoding="utf-8", errors="ignore") as f:
        text = f.read()
    return process_pgn_text(text, source_name=os.path.basename(filepath), **kwargs)
